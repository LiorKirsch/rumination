...
def saveToCsv(mat, fileName, fmt='', delimiter=";"):
    ''' For integers: fmt='%i' '''
    if (fmt==''):
        np.savetxt('%s.csv' % fileName, mat, delimiter=delimiter)
    else:
        np.savetxt('%s.csv' % fileName, mat, fmt=fmt, delimiter=delimiter)
...
def printTtestResult(A, B, Aname, Bname, doPrint=True, descriptive=True):
    cond, p = ttestCond(A, B)
    if (descriptive):
        retStr = '%s(%.5f, std=%.5f #%d) %s %s(%.5f, std=%.5f #%d) (p=%.4f) (dx=%.4f)' % (Aname, np.mean(A), np.std(A), len(A), cond, Bname, np.mean(B), np.std(B),len(B), p, np.mean(A)-np.mean(B))
    else:
        retStr = '%s %s %s (p=%.4f)' % (Aname, cond, Bname, p)
    if (doPrint): print retStr
    return retStr,p

def printTtestResultDict(d, descriptive=True):
    pairs = combinations(d.keys(),2)
    ps = []
    for (k1,k2) in pairs:
        _,p = printTtestResult(d[k1], d[k2], k1, k2, descriptive)
        ps.append(p)
    return ps
...
def calcCronbachAlpha(itemscores):
