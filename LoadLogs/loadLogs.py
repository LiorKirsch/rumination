...
CHAINS_SHORT,CHAINS_LONG = range(2) # 6 or 12
CHAINS_LEN = CHAINS_LONG

dataFolder = 'amt2' if TEST==TEST_RUMI else 'amt3' if CHAINS_LEN==CHAINS_LONG else 'amt4'

FOLDERS = ('/Users/noampeled/Dropbox/postDocMoshe/rumination/MasonWordsSite/MasonWordsSite/logs/{}/'.format(dataFolder), 
           '/home/noam/Dropbox/postDocMoshe/rumination/MasonWordsSite/MasonWordsSite/logs/{}/'.format(dataFolder))
...
MIN_GAME_TD = 5
CHAINS_TIME = 15.6
MIN_WORDS_TD = CHAINS_TIME + 5

BEST_COMB = [0, 3, 4, 5, 6, 7] if TEST==TEST_RUMI else range(10)
ALL_CATEGORIES = range(-1,4)
...
#     plotPValsForQNum(qRange,np.mean(psTrains,0),'train',np.std(psTrains,0))
    ...
    utils.save(minCombsAllTrain, 'minCombsAllTrain.pkl')
    calcCronbachAlpha(ruminationScores, categoryIDs, minCombsAllTrain)
...
def calcCronbachAlpha(logs):
    ruminationScores,categoryIDs = calcRuminationScoresAndCategories(logs,ALL_CATEGORIES,('getAllScores','normalize'))
    minCombsAllTrain = utils.load('minCombsAllTrain.pkl')
    utils.saveToCsv(ruminationScores, 'ruminationScoresForCronbachAlpha', fmt='%i')
    alphas = []
    for comb in minCombsAllTrain[1:]:
        scores = ruminationScores[:,comb]
        alpha = utils.calcCronbachAlpha(scores)
        alphas.append(alpha)
    plots.graph(range(2,ruminationScores.shape[1]+1),alphas, ylabel='Cronbach Alpha',xlabel='#Questions')
...
def calcRuminationScoresAndCategories(logs, categoriesToDiffretiate = [1,2], readLogsOptions='getAllScores'):
    ...
    for log in logs:
        ...
        if (cat in categoriesToDiffretiate):
            ...
            if (isValidLog(log,errors)):
                validLogs=validLogs+1
                rumiScores = readLog(log['uuid'],['rumination'],readLogsOptions)['rumination']
                ruminationScores.append(rumiScores)
                categoryIDs.append(cat)
    print('{} valid logs out of {}'.format(validLogs,totalLogs))
    print(errors)
    ruminationScores = np.array(ruminationScores).astype(np.int32)
...
def sumRuminationScores(scores,posQ=POS_Q,negQ=NEG_Q,comb=None):
    ...
    if (TEST==TEST_RUMI):
        ...
        combScores = np.hstack((scores[:,negIndices],scores[:,posIndices]))
    return np.sum(combScores,1)
...
# #     plots.histCalcAndPlot(clicks,min=14,max=clicksMax, binsNum=50, title='mason words clicks diffs')
    clicks = np.array(clicks) - CHAINS_TIME # 15.6 is the time it take to present a words chain
    maxTimeDiffs = np.array(maxTimeDiffs) - CHAINS_TIME
#     plots.plotCDF(clicks,(0,20),threshold=MIN_WORDS_TD-CHAINS_TIME)
#     plots.histCalcAndPlot(clicks, min=0, max=10, binsNum=50)
#     plots.plotCDF(maxTimeDiffs,threshold=MIN_WORDS_TD-CHAINS_TIME)
...
def barPlotCategoriesSocres(logs):
    ...
    if (TEST==TEST_RUMI): combs = zip([BEST_COMB],['Best questions'])
    elif (TEST==TEST_PANAS): combs = zip([range(ruminationScores.shape[1]),POS_Q,NEG_Q],['all','positive','negative'])
    for comb, combType in combs:
        print(combType)
        ruminationScoresSum = sumRuminationScores(ruminationScores,comb=comb)
        ruminationScoresSum = ruminationScoresSum / float(len(comb)) # calc the mean PANAS
        x,err=[],[]
        data={}
        for cat in catRange:  
        ...
        utils.printTtestResultDict(data)
        ylabel = 'rumination score' if TEST==TEST_RUMI else 'PANAS score' if  TEST==TEST_PANAS else ''
        plots.barPlot(x, ylabel, xTickLabels=[CATEGORIES[cat] for cat in catRange],errors=err, title=combType)

def checkWordsTimeDiff(logs):
    global MIN_WORDS_TD, MIN_GAME_TD
    catRange = [1,2]
    ps = []
    labels = [CATEGORIES[cat] for cat in catRange]
    comb = POS_Q if TEST == TEST_PANAS else BEST_COMB
    td_range = MIN_WORDS_TD + np.linspace(0, 49, 10)
    x=np.zeros((len(catRange),len(td_range)))
    err = np.zeros(x.shape)
    x.fill(np.nan)
    err.fill(np.nan)
#     ruminationScoresControl,_ = calcRuminationScoresAndCategories(logs,[3])
#     ruminationScoresSumControl = sumRuminationScores(ruminationScoresControl,comb=comb)
#     ruminationScoresSumControl = ruminationScoresSumControl / float(len(comb)) 
#     d['control'] = ruminationScoresSumControl
    for t,MIN_WORDS_TD in enumerate(td_range):
        ruminationScores,categoryIDs = calcRuminationScoresAndCategories(logs,catRange)
        ruminationScoresSum = sumRuminationScores(ruminationScores,comb=comb)
        ruminationScoresSum = ruminationScoresSum / float(len(comb)) # calc the mean PANAS
        d = {}
        for catID, cat in enumerate(catRange):
            data = ruminationScoresSum[categoryIDs==cat]
            d[labels[catID]]=data
            if (len(data)>0):  
                x[catID,t] = np.mean(data)
                err[catID,t] = np.std(data)/len(data)
        p = utils.printTtestResultDict(d)
        ps.append(p[0])
    plots.graphN(td_range, x, labels, err, doShow=False)
    plots.graph(td_range, ps)
