...
def boost(x, y, pickRand=True):
    ...
    minorityLabel,majorityLabel = (0,1) if actionsCount[0]<actionsCount[1] else (1,0)
    boostFactor = (float(actionsCount[majorityLabel]) / float(actionsCount[minorityLabel])) - 1     
    if (boostFactor>0):
...
def boostSmote(x, y, minorityLabel, k, boostFactor= -1, similarity=0.5, pickRand=True):
    majorityLabel = 1 - minorityLabel    
    cnt = Counter(y)
    if (len(cnt)<2):
        return x,y
    if (boostFactor == -1):
        boostFactor = cnt[majorityLabel] / float(cnt[minorityLabel]) - 1
