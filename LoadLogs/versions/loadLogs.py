...
FOLDERS = ('/Users/noampeled/Dropbox/postDocMoshe/rumination/MasonWordsSite/MasonWordsSite/logs/amt2', 
            '/home/noam/Dropbox/postDocMoshe/rumination/MasonWordsSite/MasonWordsSite/logs/amt2/')
#            '/home/noam/Documents/rumination/amt2')
...
CATEGORIES = {0:'control',1:'narrow',2:'wide',3:'random'}
POS_Q = [2,4,6,8]
NEG_Q = [0,1,3,5,7]
...
def fitRuminationScores(logs):
    ...
    ruminationScores[:,POS_Q] = 5-ruminationScores[:,POS_Q]
...
# #     plots.histCalcAndPlot(clicks,min=14,max=clicksMax, binsNum=50, title='mason words clicks diffs')
    plots.plotCDF(clicks,(14,50))

def calcRumiScoresHist(logs):
    ruminationScores=[]
    validRuminationScores,_ = calcRuminationScoresAndCategories(logs)
    for log in logs:
        ruminationScores.append(log['rumination'])
    ruminationScores = np.array(ruminationScores)
    validRuminationScores = sumRuminationScores(validRuminationScores)
    plots.plotCDF(validRuminationScores, title='valid rumination scores CDF')
...
def calcGameClicksHist(logs):
    ...
    for log in logs:
        if (isValidClicks(log) and isValidMT(log) and isValidRumi(log)):
            ...
            if (data['max_time_diff']<20): notValid+=1
...
#                 print(max(np.cumsum(data['times_diff'])),data['max_time_diff'],data['times_diff'][0],data['times_diff'][-1],data['clicks'])
    ...
    plots.plotCDF(timeDiffs,xlim=[0,maxTD])
